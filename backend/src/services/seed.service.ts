...
export async function seed() {
  ...
  const agency = await prisma.user.create({
    data: {
      ...
      password: await bcrypt.hash('Agency@123456', 12),
      role: 'client',
  ...
  await prisma.clientProfile.create({
    data: { userId: client.id, companyName: 'شركة النجاح للتسويق', companySize: '50-100', industry: 'تسويق رقمي', country: 'SA', city: 'الرياض' },
  });
  await prisma.clientProfile.create({
    data: { userId: agency.id, companyName: 'وكالة الإبداع للتسويق', companySize: '10-50', industry: 'وكالة إعلان', country: 'SA', city: 'جدة' },
  });

  const users = [admin, client, agency, test];
  for (const user of users) {
    await prisma.userCredits.create({
      data: { userId: user.id, balance: user.role === 'admin' ? 10000 : 500, totalSpent: 0, totalPurchased: user.role === 'admin' ? 10000 : 500 },
    });
  }

  await prisma.subscription.create({
    data: { userId: client.id, plan: 'professional', status: 'active', currentPeriodStart: daysAgo(30), currentPeriodEnd: daysLater(30) },
  });
  ...
  for (const p of platforms) {
    await prisma.platformConnection.create({
      data: { userId: p.userId, platform: p.platform, accessToken: p.token, platformAccountId: p.accountId, platformAccountName: p.name, status: 'active' },
    });
  }
  ...
  for (const c of campaignData) {
    const campaign = await prisma.campaign.create({
      data: {
        userId: c.userId, name: c.name, platform: c.platform, objective: c.objective, status: c.status,
        budgetType: 'daily', budgetAmount: c.budget, budgetCurrency: 'USD',
        startDate: c.startDate, endDate: c.endDate,
        impressions: randomInt(10000, 200000), clicks: randomInt(500, 10000), conversions: randomInt(10, 500),
        spend: randomFloat(500, 5000), ctr: randomFloat(1, 5), cpc: randomFloat(0.5, 3),
        cpm: randomFloat(5, 20), cpa: randomFloat(10, 100), revenue: randomFloat(1000, 50000), roas: randomFloat(1, 5),
      },
    });
  ...
  for (const campaign of campaigns.slice(0, 5)) {
    for (const tmpl of adTemplates) {
      await prisma.ad.create({
        data: {
          campaignId: campaign.id, platform: campaign.platform,
          name: `${tmpl.name} - ${campaign.name}`, status: campaign.status === 'active' ? 'active' : 'paused',
          creativeText: tmpl.text, creativeHeadline: tmpl.headline, creativeCta: tmpl.cta,
          impressions: randomInt(1000, 50000), clicks: randomInt(50, 3000), conversions: randomInt(1, 50),
          spend: randomFloat(50, 1000), ctr: randomFloat(1, 8), cpc: randomFloat(0.3, 3), cpm: randomFloat(3, 15), cpa: randomFloat(5, 80),
        },
      });
    }
  }
  ...
  for (const campaign of campaigns) {
    for (let day = 0; day < 30; day++) {
      ...
      await prisma.adSnapshot.create({
        data: {
          campaignId: campaign.id, platform: campaign.platform, date: daysAgo(day),
  ...
  for (const item of contentItems) {
    await prisma.content.create({
      data: { userId: item.userId, type: item.type, title: item.title, description: item.description, fileUrl: `/uploads/${item.type}s/${uuidv4()}.jpg`, tags: JSON.stringify(item.tags) },
    });
  }
  ...
  for (const comp of competitorData) {
    await prisma.competitor.create({
      data: { userId: comp.userId, name: comp.name, platform: comp.platform, platformUsername: comp.username, notes: comp.notes, estimatedSpend: comp.spend, activeAdsCount: randomInt(3, 15) },
    });
  }

  const reports = [
    { userId: client.id, productName: 'منصة تسويق رقمي', category: 'تكنولوجيا', country: 'السعودية', summary: 'سوق التسويق الرقمي في السعودية ينمو بنسبة 15% سنوياً' },
    { userId: client.id, productName: 'متجر إلكتروني', category: 'تجارة إلكترونية', country: 'الإمارات', summary: 'التجارة الإلكترونية في الإمارات تشهد نمواً متسارعاً' },
  ];
  for (const r of reports) {
    await prisma.marketReport.create({
      data: {
        userId: r.userId, productName: r.productName, productCategory: r.category, country: r.country,
        reportData: JSON.stringify({
          marketSize: '5 مليار دولار', growth: '15% سنوياً',
  ...
  for (const agent of agentTypes) {
    await prisma.aiAgent.create({
      data: { userId: client.id, name: agent.name, type: agent.type, systemPrompt: agent.prompt, model: 'gpt-4o-mini', temperature: 0.7, maxTokens: 2000, isActive: true },
    });
  }

  const inbox = await prisma.socialInbox.create({
    data: { userId: client.id, name: 'الوارد الموحد', platform: 'facebook', platformAccountId: 'fb_page_123' },
  });

  const messageTexts = [
    'مرحباً، كم سعر المنتج الجديد؟', 'هل لديكم خصومات للشركات الناشئة؟', 'أرغب في طلب استشارة تسويقية',
    'شكراً لكم على الخدمة الممتازة', 'هل توفرون خدمات التصميم؟', 'متى موعد إطلاق الحملة الجديدة؟',
    'أريد معرفة المزيد عن الباقة الاحترافية', 'هل الشحن مجاني للطلبات الكبيرة؟',
  ];
  for (let i = 0; i < 8; i++) {
    await prisma.socialMessage.create({
      data: {
        inboxId: inbox.id, userId: client.id, platform: 'facebook',
        direction: i < 5 ? 'inbound' : 'outbound', status: i < 3 ? 'unread' : 'read',
        senderName: `عميل ${i + 1}`, senderId: `user_${i + 1}`, messageText: messageTexts[i], createdAt: daysAgo(i),
      },
    });
  }
  ...
  await prisma.organization.create({
    data: { name: 'وكالة الإبداع للتسويق', companySize: '10-50', industry: 'وكالة إعلان' },
  });

  await prisma.creditTransaction.create({ data: { userId: client.id, amount: 500, type: 'purchase', description: 'شراء حزمة 500 توكن', balanceAfter: 500 } });
  await prisma.creditTransaction.create({ data: { userId: client.id, amount: -10, type: 'spend', description: 'توليد محتوى باستخدام AI', balanceAfter: 490 } });
  await prisma.creditTransaction.create({ data: { userId: client.id, amount: -5, type: 'spend', description: 'تحليل حملة إعلانية', balanceAfter: 485 } });
  await prisma.creditTransaction.create({ data: { userId: client.id, amount: 100, type: 'bonus', description: 'مكافأة ترحيبية', balanceAfter: 600 } });
